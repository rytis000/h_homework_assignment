...
@app.get("/generate_domain_name/{business_description}")
async def generate_domain_names(business_description: str) -> list[str]:
    try:
        test_description = f"A business that specializes in {business_description}."
        suggestions = generate_domain_suggestions(model, tokenizer, test_description)
        response = suggestions.split("Suggested domains: ")[1].split(", ")
        return response[:-1]
